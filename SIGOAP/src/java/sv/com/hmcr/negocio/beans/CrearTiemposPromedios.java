...
@ManagedBean
@ViewScoped
public class CrearTiemposPromedios implements java.io.Serializable{
    ...
    public void generar() {
        ...
        parametrosReportes.setEncabezado("HMCR SOLUTIONS\nTIEMPOS PROMEDIOS DE PROCESAMIENTO DE ORDENES\nDEL PERIODO "+fecIni+" AL "+fecFin+"\n\nSUPERVISADOS POR:"+getParametrosReportes().getSupervisor());
        getDao().ejecutarProc("CALL tiemposPromedios('" + fecIni + "','" + fecFin + "','"+getParametrosReportes().getSupervisor()+"')");
        try {
            FacesContext contex = FacesContext.getCurrentInstance();
            contex.getExternalContext().redirect("tiempoPromedio.xhtml");
        } catch (Exception e) {
        }
    }
    /**
